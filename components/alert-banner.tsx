...
export function AlertBanner() {
  ...
  useEffect(() => {
    ...
    const shouldShowBanner = isLoggedIn && !isPro && freeProMessagesLeft <= AI_REQUEST_FREE_TIER_LIMIT;
  ...
  const getBannerContent = () => {
    ...
    if (isLoggedIn && !isPro && freeProMessagesLeft <= 0) {
      return (
        <span>
          Upgrade to <span className="text-primary font-medium">Pro</span> to unlock unlimited requests.
        </span>
      );
    }
  };
  ...
  return (
    <div className="@container/alert-banner relative">
      ...
      <div className={cn("relative grid content-end transition-all duration-300 ease-in-out", showBanner ? "grid-rows-[1fr]" : "grid-rows-[0fr]")}>
        <div className="overflow-hidden">
          <div className="bg-muted text-muted-foreground flex items-center justify-between gap-2 rounded-t-lg px-3 py-1.5 text-xs">
            <p className={cn("text-pretty @2xl/alert-banner:text-sm", showBanner ? "opacity-100" : "opacity-0")}>{getBannerContent()}</p>
            <div className="ml-auto flex items-center gap-1">
              ...
              <Button variant="ghost" size="icon" className="size-4 [&>svg]:size-3" onClick={() => setShowBanner(false)}>
                <X />
              </Button>
            </div>
          </div>
